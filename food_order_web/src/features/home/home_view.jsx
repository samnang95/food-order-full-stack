...
export function HomeView() {
  ...
  return (
    <div className="space-y-12 sm:space-y-16 pb-16">
      {/* 1. Hero Section */}
      <section className="relative overflow-hidden rounded-3xl sm:rounded-[2.5rem] bg-gradient-to-br from-orange-500 via-amber-500 to-rose-500 text-white p-6 sm:p-12 lg:p-16 shadow-2xl shadow-orange-500/20">
        {/* Ambient decorative elements */}
        <div className="absolute top-0 right-0 -mt-12 -mr-12 w-96 h-96 rounded-full bg-white/10 blur-3xl pointer-events-none" />
        <div className="absolute bottom-0 left-0 -mb-12 -ml-12 w-80 h-80 rounded-full bg-black/10 blur-2xl pointer-events-none" />

        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          {/* Hero Left Content */}
          <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
            {/* Promo Tag */}
            <div className="inline-flex items-center space-x-2 bg-white/20 backdrop-blur-md px-3.5 py-1.5 rounded-full text-xs font-black uppercase tracking-wider text-amber-100 border border-white/25">
              <span>🏷️ Promo</span>
            ...
            <p className="text-sm sm:text-base text-orange-50/90 max-w-xl mx-auto lg:mx-0 leading-relaxed font-medium">
            ...
            {/* Hero Search Bar */}
            <form onSubmit={handleHeroSearch} className="max-w-lg mx-auto lg:mx-0 flex items-center bg-white p-2 rounded-2xl shadow-xl shadow-orange-950/20 text-slate-800">
              <span className="pl-3 text-slate-400 text-lg">🔍</span>
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="What are you craving today? (e.g. Smash Burger, Ramen...)"
                className="flex-1 px-3 py-2 text-xs sm:text-sm text-slate-900 placeholder-slate-400 focus:outline-hidden"
              />
              <button
                type="submit"
                className="px-5 py-2.5 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-black text-xs sm:text-sm transition-colors shadow-md"
              >
                Search
              </button>
            </form>
            ...
            {/* Trust Badges */}
            <div className="pt-2 flex flex-wrap items-center justify-center lg:justify-start gap-4 text-xs font-bold text-orange-100">
              <div className="flex items-center space-x-1.5 bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-full">
                <span>⚡</span>
          ...
          <div className="lg:col-span-5 relative flex justify-center">
            <div className="relative w-64 h-64 sm:w-80 sm:h-80 lg:w-96 lg:h-96">
              ...
              <div className="absolute -bottom-2 -left-4 z-20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md p-3.5 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white flex items-center space-x-3">
                <span className="text-2xl">🔥</span>
                <div>
                  <p className="text-xs font-bold">10,000+ Orders</p>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400">Delivered Across Phnom Penh</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs flex items-start space-x-4">
          <div className="w-12 h-12 rounded-2xl bg-orange-100 dark:bg-orange-950/60 text-orange-600 flex items-center justify-center text-2xl shrink-0">
            🛵
          </div>
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
              Dispatched with live GPS updates. Hot food arrives at your door in an average of 28 minutes.
            </p>
          </div>
        </div>

        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs flex items-start space-x-4">
          <div className="w-12 h-12 rounded-2xl bg-amber-100 dark:bg-amber-950/60 text-amber-600 flex items-center justify-center text-2xl shrink-0">
            🌱
          </div>
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
              Every burger, pizza, and noodle bowl is crafted upon ordering with farm-fresh ingredients.
            </p>
          </div>
        </div>

        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs flex items-start space-x-4">
          <div className="w-12 h-12 rounded-2xl bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 flex items-center justify-center text-2xl shrink-0">
            📱
          </div>
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white">
